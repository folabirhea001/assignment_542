...
Placeholders replaced:
    2021/1/84096CF, 2021-1-84096CF, Afolabi Victoria Boluwatife, victoria.m2104190@st.futminna.edu.ng,
    Information Technology, https://github.com/folabirhea001/assignment_542.git, 2026-08-13
"""
import argparse
import datetime
import json
...
def sanitise_matric(matric: str) -> str:
    ...
    if not m:
        fail(f"matric_no '{matric}' does not match the FUT Minna format "
             f"(e.g. 2024/1-12345 or 2021/1/84096CF)")
...
def replace_placeholders(dst: Path, values: dict) -> int:
    ...
    for path in dst.rglob('*'):
        ...
        if '{{' in text:
            ...
            replaced += 1
        for old, new in LITERAL_REPLACEMENTS(values).items():
            if old in text:
...
def LITERAL_REPLACEMENTS(values: dict) -> dict:
    return {
        '2021-1-84096CF':            values['MATRIC_NO_FILE'],
        '2021/1/84096CF':            values['MATRIC_NO'],
        'Afolabi Victoria Boluwatife':            values['STUDENT_NAME'],
        'victoria.m2104190@st.futminna.edu.ng':  values['STUDENT_EMAIL'],
        'Information Technology':    values['DEPARTMENT'],
        'https://github.com/folabirhea001/assignment_542.git': values['GITHUB_URL'],
        '2026-08-13':                values['DATE'],
    }
